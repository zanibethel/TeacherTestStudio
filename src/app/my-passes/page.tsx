...
export default async function MyPasses(){
  ...
  const metrics=await Promise.all(rows.map(async(p:any)=>{
    const[{data:readiness},{data:progress}]=await Promise.all([
      ...
      supabase.rpc('get_bundle_progress',{p_bundle_id:p.bundle_id})
    ])
    return[p.bundle_id,{readiness,progress}] as const
  }))
  const metricsByBundle=new Map(metrics)
  ...
  const PassCard=({p}:{p:any})=>{const metric:any=metricsByBundle.get(p.bundle_id)||{};const readiness=metric.readiness;const progress=metric.progress;const score=readiness?.readiness_score==null?null:Number(readiness.readiness_score);return <section className="card">
    ...
    <div className="grid three pass-stats">
      ...
      <div><span className="muted">Progress</span><b>{Number(progress?.completed_sessions||0)>1?signed(progress?.overall_change):'—'}</b></div>
      <div><span className="muted">Completed</span><b>{p.completed_sessions??0}</b></div>
    </div>
    {score!=null?<p><b>{readinessLabel(score)}</b>{progress?.attention_area?<> · Priority: <b>{progress.attention_area}</b></>:readiness?.weakest_area&&<> · Next focus: <b>{readiness.weakest_area}</b></>}</p>:<p className="muted">Complete practice to generate your readiness score and next-focus recommendation.</p>}
    {progress?.strongest_improvement_area&&Number(progress?.completed_sessions||0)>1&&<p className="good">Strongest improvement: <b>{progress.strongest_improvement_area}</b> {signed(progress.strongest_improvement_change)}</p>}
    ...
    <div className="row"><Link className="button" href={`/practice-library/bundles/${p.bundle_id}`}>{p.access_state==='active'?(progress?.attention_area?'Practice priority area':'Continue practice'):p.access_state==='pending'?'View selection':'View bundle'}</Link></div>
  </section>}
