...
export async function createShareOffer(testId:string,fd:FormData){
  ...
  const requested=String(fd.get('delivery_mode')||'standard');const deliveryMode=['standard','restricted','study','paid_pass'].includes(requested)?requested:'standard';const paid=deliveryMode==='paid_pass';if(paid&&!proActive)redirect(`/tests/${testId}?error=${encodeURIComponent('Teacher Pro is required to create paid practice passes.')}`)
  const attemptsRaw=Math.max(1,Math.min(5,Number(fd.get('max_attempts')||1)));const durationRaw=Math.max(1,Math.min(365,Number(fd.get('access_duration_days')||14)));const priceRaw=Number(fd.get('price_dollars')||0);if(paid&&priceRaw<=0)redirect(`/tests/${testId}?error=${encodeURIComponent('Enter a price for the paid practice pass.')}`)
  ...
  const expiresRaw=String(fd.get('link_expires_at')||'').trim();const guide=fd.get('study_guide_enabled')==='on'||deliveryMode==='study'||paid;const focused=fd.get('focused_retake_enabled')==='on'||deliveryMode==='study'||paid;const randomized=fd.get('randomized_retest_enabled')==='on'||paid
  const payload={test_id:testId,teacher_id:user.id,label:String(fd.get('label')||'').trim()||null,delivery_mode:deliveryMode,restricted_mode:deliveryMode==='restricted',access_mode:paid?'practice_pass':'classroom',payment_mode:paid?'paid':'free',max_attempts:paid?null:attemptsRaw,access_duration_days:paid?durationRaw:null,study_guide_enabled:guide,focused_retake_enabled:focused,randomized_retest_enabled:randomized,link_expires_at:expiresRaw?new Date(expiresRaw).toISOString():null,price_cents:paid?Math.round(priceRaw*100):null,audience_mode:audienceMode}
