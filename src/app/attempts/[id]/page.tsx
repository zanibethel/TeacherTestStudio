...
export default async function AttemptReview({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: attempt } = await supabase.from('attempts').select('id,student_id,score_percent,correct_count,total_questions,submitted_at,test_id,tests(title,teacher_id),student:profiles!attempts_student_id_fkey(full_name),responses(id,is_correct,choice_id,questions(prompt,position,choices(id,label,position)))').eq('id',id).single()
  if (!attempt) notFound()
