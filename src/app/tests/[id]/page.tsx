...
export default async function TestDetail({params,searchParams}:{params:Promise<{id:string}>,searchParams:Promise<{error?:string}>}){
  ...
  const{data:shares}=await supabase.from('test_shares').select('id,token,label,delivery_mode,restricted_mode,access_mode,payment_mode,max_attempts,access_duration_days,study_guide_enabled,link_expires_at,price_cents,teacher_revenue_share_bps,active,created_at').eq('test_id',id).order('created_at',{ascending:false})
  ...
  return <main>
    ...
    <section className="card"><h2>Publish</h2><p>Legacy classroom code: <b className="code">{test.share_code}</b></p><p><a href={shareUrl}>{shareUrl}</a></p><p className="muted">New share links below can give this same test different rules without rebuilding it.</p><p>Question order: <b>{test.randomize_questions?'Randomized':'Fixed'}</b></p><form action={setTestStatus.bind(null,id,test.status==='published'?'draft':'published')}><button>{test.status==='published'?'Unpublish test':'Publish test'}</button></form></section>

    <section className="card stack"><div><h2>Share this test</h2><p className="muted">Choose how this particular link should behave. The underlying test stays the same.</p></div>
      <form action={createShareOffer.bind(null,id)} className="stack">
        <label>Share label</label><input name="label" placeholder="Week 1 exam, study retake, 14-day PSI pass"/>
        <label>How should students use this share?</label>
        <select name="delivery_mode" defaultValue="standard">
          <option value="standard">Standard test — 1–5 attempts, score/results only</option>
          <option value="restricted">Restricted Test Mode — 1–5 attempts + integrity monitoring</option>
          <option value="study">Study / outline mode — 1–5 attempts + study guide</option>
          <option value="paid_pass">Paid access pass — Pro only, unlimited attempts + study guides</option>
        </select>
        <div className="settings-grid"><div><label>Attempts</label><input name="max_attempts" type="number" min="1" max="5" defaultValue="1"/><p className="muted">Used by Standard, Restricted, and Study shares. Paid passes use unlimited attempts.</p></div><div><label>Optional link expiration</label><input name="link_expires_at" type="datetime-local"/><p className="muted">Leave blank if the share link itself should remain available.</p></div></div>
        <div className="settings-grid"><div><label>Paid pass length</label><input name="access_duration_days" type="number" min="1" max="365" defaultValue="14"/><p className="muted">The clock starts when the student activates the paid pass.</p></div><div><label>Paid pass price</label><input name="price_dollars" type="number" min="1" step="0.01" placeholder="12.99"/><p className="muted">{proActive?'Your account can create paid passes. Teacher revenue share will be applied when checkout/payouts are connected.':'Upgrade to Teacher Pro to create paid passes.'}</p></div></div>
        <button>Create share link</button>
      </form>
    </section>

    {(shares??[]).length>0&&<section className="card"><h2>Share links</h2>{(shares??[]).map((s:any)=>{const url=`https://teacher-test-studio.vercel.app/share/${s.token}`;const mode=s.delivery_mode==='paid_pass'?'Paid access pass':s.delivery_mode==='study'?'Study / outline':s.delivery_mode==='restricted'?'Restricted test':'Standard test';return <div className="question-summary" key={s.id}><div className="row between"><div><b>{s.label||'Untitled share'}</b><p><a href={url}>{url}</a></p><p className="muted">{mode} · {s.max_attempts==null?'Unlimited attempts':`${s.max_attempts} attempt${s.max_attempts===1?'':'s'}`} · Study guide {s.study_guide_enabled?'on':'off'}{s.restricted_mode?' · Integrity monitoring on':''}{s.payment_mode==='paid'?` · $${((s.price_cents||0)/100).toFixed(2)} · ${s.access_duration_days||14}-day access`:''}{s.link_expires_at?` · Link closes ${new Date(s.link_expires_at).toLocaleString()}`:''}</p>{s.payment_mode==='paid'&&<p className="muted">Teacher revenue share: {s.teacher_revenue_share_bps>0?`${(s.teacher_revenue_share_bps/100).toFixed(2)}%`:'percentage to be set when payouts launch'}</p>}</div><span className="pill">{s.active?'Active':'Disabled'}</span></div><form action={setShareActive.bind(null,id,s.id,!s.active)}><button className="secondary">{s.active?'Disable link':'Enable link'}</button></form></div>})}</section>}
