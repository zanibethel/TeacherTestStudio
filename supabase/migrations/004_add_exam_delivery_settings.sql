alter table public.tests
  add column if not exists duration_minutes integer not null default 120 check (duration_minutes between 0 and 600),
  add column if not exists one_question_per_page boolean not null default true,
  add column if not exists passing_score_percent integer not null default 70 check (passing_score_percent between 0 and 100),
  add column if not exists exam_preset text not null default 'custom';

create or replace function public.create_test_with_questions_v2(
  p_title text,
  p_description text,
  p_randomize boolean,
...
declare
  ...
  q jsonb; c jsonb; q_index int := 0; c_index int; correct_index int;
begin
  ...
  insert into public.tests(teacher_id,title,description,randomize_questions,duration_minutes,one_question_per_page,passing_score_percent,exam_preset)
  values(v_user,trim(p_title),nullif(trim(p_description),''),coalesce(p_randomize,false),p_duration_minutes,coalesce(p_one_question_per_page,true),p_passing_score,coalesce(nullif(trim(p_exam_preset),''),'custom')) returning id into v_test;
  for q in select value from jsonb_array_elements(p_questions) loop
    ...
    insert into public.questions(test_id,prompt,position) values(v_test,trim(q->>'prompt'),q_index) returning id into v_question;
...
$$;
revoke all on function public.create_test_with_questions_v2(text,text,boolean,integer,boolean,integer,text,jsonb) from public, anon;
grant execute on function public.create_test_with_questions_v2(text,text,boolean,integer,boolean,integer,text,jsonb) to authenticated;
