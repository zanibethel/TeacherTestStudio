...
export default async function SharedTest({params,searchParams}:{params:Promise<{token:string}>,searchParams:Promise<{error?:string;fresh?:string}>}){
  ...
  const{data:test}=await supabase.from('tests').select('id,title,description,status,teacher_id,randomize_questions,randomize_choices,duration_minutes,one_question_per_page,passing_score_percent,allow_save_resume,strict_mode,integrity_action,integrity_limit,questions(id,prompt,position,choices(id,label,position))').eq('id',share.test_id).eq('status','published').single();if(!test)notFound()
  const{data:teacher}=await supabase.rpc('get_teacher_public_profile',{p_teacher_id:test.teacher_id})
  const{data:attemptId,error:startError}=await supabase.rpc('start_shared_test_attempt',{p_share_token:token});if(startError)return <main className="narrow"><h1>{test.title}</h1><p className="bad">{startError.message}</p><Link className="button" href="/dashboard">Back to dashboard</Link></main>
  ...
  const questions=ordered.map((q:any)=>({...q,choices:(test.randomize_choices||fresh)?shuffled(q.choices??[]):[...(q.choices??[])].sort((a:any,b:any)=>a.position-b.position)}))
