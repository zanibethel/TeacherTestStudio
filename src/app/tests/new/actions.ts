...
export async function createTest(formData: FormData) {
  ...
  if (error) redirect('/tests/new?error=' + encodeURIComponent(error.message));await persistQuestionTags(supabase,user.id,questions);revalidatePath('/question-bank');revalidatePath('/tests/new');redirect(`/tests/${data}/preview?created=1`)
}
