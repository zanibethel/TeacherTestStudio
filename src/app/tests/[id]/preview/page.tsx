...
export default async function TestPreview({params,searchParams}:{params:Promise<{id:string}>,searchParams:Promise<{created?:string,error?:string,source?:string}>}){
 ...
 return <main>
  ...
  <section className="card" style={{padding:'14px 18px'}}><div className="row" style={{gap:8}}><form action={generateAlternateVersion.bind(null,id)}><button className="secondary" style={{padding:'8px 11px'}} type="submit">↻ New version</button></form><Link className="secondary button" style={{padding:'8px 11px'}} href={`/tests/${id}/edit`}>✎ Edit</Link></div></section>
