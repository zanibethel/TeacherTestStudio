...
export default function ShareSetupForm({roster,groups,presets,proActive,fullQuestionCount}:{roster:RosterRow[];groups:GroupRow[];presets:SavedPreset[];proActive:boolean;fullQuestionCount:number}){
  const options=useMemo<Option[]>(()=>[
    ...
    {key:'restricted',name:'Restricted test',settings:RESTRICTED},
    {key:'study',name:'Study + remediation',settings:STUDY},
    {key:'paid',name:'Paid practice · Pro',settings:PAID},
  ...
  const attemptSummary=settings.unlimited?'Unlimited until due date':`${settings.maxAttempts} full attempt${settings.maxAttempts===1?'':'s'}`
  const remediationSummary=settings.requireFocused?'Focused retest required':settings.focusedRetake?'Weak-area practice available':'Off'
  const learningSummary=[settings.studyGuide?'Study guide':null,settings.focusedRetake?'Weak-area practice':null].filter(Boolean).join(' + ')||'Off'
