...
export async function updateBankQuestion(id:string,fd:FormData){
  ...
  const prompt=String(fd.get('prompt')||'').trim();const contentArea=String(fd.get('content_area')||'').trim();const explanation=String(fd.get('explanation')||'').trim()
  ...
  const{error}=await supabase.from('question_bank').update({prompt,normalized_prompt:normalize(prompt),choices,correct_index:correctIndex,content_area:contentArea||null,explanation:explanation||null,updated_at:new Date().toISOString()}).eq('id',id).eq('teacher_id',user.id)
