...
export default async function TakeTest({ params, searchParams }: { params: Promise<{ id: string }>, searchParams: Promise<{ error?: string }> }) {
  ...
  const { data: test } = await supabase.from('tests').select('id,title,description,status,randomize_questions,randomize_choices,duration_minutes,one_question_per_page,passing_score_percent,allow_save_resume,strict_mode,integrity_action,integrity_limit,questions(id,prompt,position,choices(id,label,position))').eq('id',id).eq('status','published').single()
  if (!test) notFound()

  const { data: attemptId, error: startError } = await supabase.rpc('start_test_attempt',{p_test_id:id})
  if (startError) return <main><h1>{test.title}</h1><p className="bad">{startError.message}</p><a className="button" href="/dashboard">Back to dashboard</a></main>
  const { data: attempt } = await supabase.from('attempts').select('id,deadline_at,attempt_number,responses(question_id,choice_id)').eq('id',attemptId).single()
  if (!attempt) notFound()

  const initialAnswers: Record<string,string> = {}
  for (const r of (attempt.responses ?? []) as any[]) if (r.choice_id) initialAnswers[r.question_id] = r.choice_id
  ...
  return <>
    ...
    <ExamRunner
      ...
      allowSaveResume={test.allow_save_resume ?? false}
      strictMode={test.strict_mode ?? false}
      integrityAction={test.integrity_action ?? 'flag'}
      integrityLimit={test.integrity_limit ?? 3}
      initialAnswers={initialAnswers}
      action={submitTest.bind(null,attempt.id,id)}
      saveAction={saveProgress.bind(null,attempt.id)}
      integrityActionCall={logIntegrity.bind(null,attempt.id)}
    />
  </>
}
