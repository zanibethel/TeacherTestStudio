...
import { deleteBankQuestion,refreshSharedBankQuestions } from './actions'
import QuestionBankBrowser from './QuestionBankBrowser'

export default async function QuestionBank({searchParams}:{searchParams:Promise<{error?:string;refreshed?:string}>}){
  ...
  return <main>
    <div className="row between" style={{alignItems:'flex-start',gap:12,flexWrap:'wrap'}}><div><Link href="/dashboard">← Dashboard</Link><h1>Question bank</h1><p className="muted">Browse by bundle and topic, or search to jump straight to matching questions.</p></div><div className="row" style={{flexWrap:'wrap'}}><Link className="secondary button" href="/shared-library">Browse shared library</Link><Link className="button" href="/tests/new">+ Build a test</Link></div></div>
