...
export default function TestBuilder({ action, bankQuestions=[] }: { action:(formData:FormData)=>void; bankQuestions?:BankQuestion[] }) {
  ...
  return <form action={action} className="stack">
    ...
    <details className="card" open><summary><b>Choose from question bank ({bankQuestions.length})</b></summary><p className="muted">Every question you save becomes reusable here. Search by wording or topic, then add it to this test.</p><input value={bankSearch} onChange={e=>setBankSearch(e.target.value)} placeholder="Search saved questions or content areas"/>{filteredBank.length===0?<p className="muted">No matching saved questions yet.</p>:<div>{filteredBank.map(item=><div className="result-row" key={item.id}><div><b>{item.prompt}</b><p className="muted">{item.content_area||'No topic'} · {item.choices.length} choices</p></div><button type="button" className="secondary" onClick={()=>addFromBank(item)}>Add</button></div>)}</div>}</details>
