...
export default async function TestBlueprints({searchParams}:{searchParams:Promise<{error?:string;saved?:string}>}){
 ...
 {blueprints?.length?<section className="card"><div className="row between"><div><h2>Saved blueprints</h2><p className="muted">Generate another version at any time. New questions added to your bank automatically become eligible.</p></div><span className="pill">{blueprints.length}</span></div><div className="blueprint-list">{blueprints.map((b:any)=><article className="blueprint-card" key={b.id}><div><h3>{b.name}</h3>{b.description&&<p>{b.description}</p>}<p className="muted">{chapterText(b.chapter_filters)}</p><p className="muted">{weightText(b.subject_weights)}</p><p className="muted">{b.question_count} questions · {b.duration_minutes?`${b.duration_minutes} min`:'Untimed'} · {b.passing_score_percent}% passing</p></div><div className="row" style={{flexWrap:'wrap'}}><form action={generateFromBlueprint.bind(null,b.id)}><button>Generate fresh draft</button></form><Link className="secondary button" href={`/test-blueprints/${b.id}/edit`}>Edit</Link><form action={deleteBlueprint.bind(null,b.id)}><button className="ghost danger">Delete</button></form></div></article>)}</div></section>:<section className="card"><h2>No saved blueprints yet</h2><p className="muted">Create one below. A blueprint stores selection rules, not a fixed set of questions.</p></section>}
 <BlueprintEditor action={saveBlueprint} questions={(questions??[]) as any}/>
 <style jsx global>{`.blueprint-list{display:grid;gap:10px}.blueprint-card{display:grid;grid-template-columns:1fr auto;gap:16px;align-items:center;border:1px solid #e4e7ef;border-radius:12px;padding:14px}.blueprint-card h3{margin:0 0 5px}.blueprint-card p{margin:3px 0}.blueprint-card form{margin:0}@media(max-width:760px){.blueprint-card{grid-template-columns:1fr}.blueprint-card .row{align-items:stretch}.blueprint-card .row>*{flex:1}.blueprint-card .row button,.blueprint-card .row a{width:100%;text-align:center}}`}</style>
 </main>
}
