...
export default async function BundleAdmin({searchParams}:{searchParams:Promise<{message?:string;error?:string;bundle?:string}>}){
  ...
  return <main>
    ...
    <div className="row between"><div><h1>Bundle manager</h1><p className="muted">Build and publish CramLoop platform bundles without touching SQL.</p></div><span className="pill">Platform admin</span></div>
    ...
    <div className="admin-bundle-layout">
      <aside className="card admin-bundle-list"><h2>Bundles</h2>{bundles.map((b:any)=><Link key={b.id} className={`admin-bundle-link ${selected?.id===b.id?'active':''}`} href={`/admin/bundles?bundle=${b.id}`}><b>{b.title}</b><span>{b.publication_status} · {b.category||'Uncategorized'}</span></Link>)}</aside>
      <div>
        {!selected?<section className="card"><p className="muted">Create your first platform bundle above.</p></section>:<>
          <section className="card">
            <div className="row between"><h2>Bundle details</h2><span className="pill">{selected.publication_status}</span></div>
            <form action={saveBundle.bind(null,selected.id)}>
              ...
              <div className="grid two">
                ...
                <label>Content version<input name="content_version" defaultValue={selected.content_version||'1.0'}/></label>
              ...
              <div className="row" style={{marginTop:16}}><button type="submit">Save bundle</button><Link href={`/practice-library/bundles/${selected.id}`}>Student view →</Link></div>
            </form>
          </section>
          ...
          <section className="card"><h2>Included resources</h2><p className="muted">Attach platform question collections and choose which ones students can preview for free.</p>
