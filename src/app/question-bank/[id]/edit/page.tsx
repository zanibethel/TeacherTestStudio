...
export default async function EditBankQuestion({params,searchParams}:{params:Promise<{id:string}>,searchParams:Promise<{error?:string}>}){
  ...
  const[{data:q},{data:allChapters},{data:allSubjects},{data:qChapters},{data:qSubjects}]=await Promise.all([
    supabase.from('question_bank').select('id,prompt,choices,correct_index,content_area,subject_category,chapter_number,chapter_title,explanation,focused_retake_hint,source_type,shared_question_id').eq('id',id).eq('teacher_id',user.id).single(),
    supabase.from('question_bank_chapters').select('chapter_number,chapter_title').eq('teacher_id',user.id).limit(2000),
    supabase.from('question_bank_subjects').select('subject_category').eq('teacher_id',user.id).limit(2000),
    supabase.from('question_bank_chapters').select('chapter_number,chapter_title').eq('question_id',id).eq('teacher_id',user.id),
    supabase.from('question_bank_subjects').select('subject_category').eq('question_id',id).eq('teacher_id',user.id)
  ]);if(!q)notFound()
  const chapters=[...new Map((allChapters??[]).map((item:any)=>{const option:ChapterOption={number:item.chapter_number??null,title:item.chapter_title??''};return[`${option.number??''}|${option.title.toLowerCase()}`,option]})).values()]
  const subjects=[...new Set((allSubjects??[]).map((item:any)=>String(item.subject_category||'').trim()).filter(Boolean))]
  const questionChapters=(qChapters??[]).map((item:any)=>({number:item.chapter_number??null,title:item.chapter_title??''}));const questionSubjects=(qSubjects??[]).map((item:any)=>String(item.subject_category||'').trim()).filter(Boolean)
  const value:QuestionEditorValue={prompt:q.prompt,choices:Array.isArray(q.choices)?q.choices.map(String):['',''],correctIndex:q.correct_index??0,chapterNumber:q.chapter_number??null,chapterTitle:q.chapter_title??'',subjectCategory:q.subject_category??q.content_area??'',focusedRetakeHint:q.focused_retake_hint??'',explanation:q.explanation??'',sourceType:q.source_type,chapters:questionChapters.length?questionChapters:undefined,subjects:questionSubjects.length?questionSubjects:undefined}
