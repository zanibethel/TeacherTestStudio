...
export default async function QuestionBank({searchParams}:{searchParams:Promise<{q?:string;area?:string;error?:string}>}){
  ...
  const query=await searchParams;let request=supabase.from('question_bank').select('id,prompt,choices,correct_index,content_area,source_type,updated_at,shared_question_id,imported_collection_id').order('updated_at',{ascending:false})
  ...
  return <main><div className="row between"><div><Link href="/dashboard">← Dashboard</Link><h1>Question bank</h1><p className="muted">Your private, editable collection. Shared-library imports become independent copies here.</p></div><div className="row"><Link className="secondary button" href="/shared-library">Browse shared library</Link><Link className="button" href="/tests/new">+ Build a test</Link></div></div>{query.error&&<p className="bad">{query.error}</p>}<form className="card" method="get"><div className="settings-grid"><div><label>Search question wording</label><input name="q" defaultValue={query.q??''} placeholder="infection control"/></div><div><label>Content area</label><select name="area" defaultValue={query.area??''}><option value="">All topics</option>{areaList.map(area=><option key={area} value={area}>{area}</option>)}</select></div></div><button>Filter</button></form><p className="muted">{questions?.length??0} saved question{questions?.length===1?'':'s'} shown</p>{!questions?.length?<section className="card"><p className="muted">Your bank is empty. Add a shared collection or save questions from a test.</p></section>:questions.map((q:any)=>{const choices=Array.isArray(q.choices)?q.choices:[];return <section className="card" key={q.id}><div className="row between"><div><b>{q.prompt}</b><p className="muted">{q.content_area||'No topic'} · {q.shared_question_id?'Copied from shared library':q.source_type}</p></div><div className="row"><Link className="secondary button" href={`/question-bank/${q.id}/edit`}>Edit my copy</Link><form action={deleteBankQuestion.bind(null,q.id)}><button className="ghost danger">Remove</button></form></div></div><ol type="A">{choices.map((choice:string,i:number)=><li key={i}><span className={i===q.correct_index?'good':''}>{choice}{i===q.correct_index?' ✓':''}</span></li>)}</ol></section>})}</main>
}
