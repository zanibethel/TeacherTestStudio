...
export default async function TestPreview({params,searchParams}:{params:Promise<{id:string}>,searchParams:Promise<{created?:string,error?:string,source?:string}>}){
 ...
 return <main>
  <div style={{marginBottom:12}}><Link href="/dashboard">← Tests</Link><h1 style={{margin:'10px 0 2px'}}>Preview</h1><p className="muted" style={{margin:0}}>Check the student view, then edit or assign.</p></div>
  ...
  <section className="card" style={{padding:'14px 18px'}}><div className="row" style={{gap:8,flexWrap:'wrap'}}><form action={generateAlternateVersion.bind(null,id)}><button className="secondary" style={{padding:'8px 11px'}} type="submit">↻ New version</button></form><Link className="secondary button" style={{padding:'8px 11px'}} href={`/tests/${id}/print`}>🖨 Print</Link><Link className="secondary button" style={{padding:'8px 11px'}} href={`/tests/${id}/edit`}>✎ Edit</Link></div></section>
