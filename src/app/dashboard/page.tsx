...
export default async function Dashboard({searchParams}:{searchParams:Promise<{error?:string}>}){
  ...
  if(profile?.role==='teacher'){
    ...
    return <main>
      ...
      <section className="card dashboard-actions" style={{padding:16}}>
        ...
        <div className="dashboard-tool-grid"><Link className="dashboard-tool" href="/classroom-tools"><b>Classroom tools</b><span>Run local PSI practice or print randomized exams</span></Link><Link className="dashboard-tool" href="/tests/new/smart"><b>Smart test</b><span>Mix multiple sources, like Chapters 1–3</span></Link><Link className="dashboard-tool" href="/question-bank"><b>Question bank</b><span>{bankCount??0} saved questions</span></Link><Link className="dashboard-tool" href="/shared-library"><b>Shared resources</b><span>Browse reusable teacher content</span></Link><Link className="dashboard-tool" href="/teacher-progress"><b>Student progress</b><span>See who needs attention now</span></Link><Link className="dashboard-tool" href="/teacher-roster"><b>Student roster</b><span>Manage your students</span></Link><Link className="dashboard-tool" href="/teacher-groups"><b>Groups</b><span>Organize classes and cohorts</span></Link>{profile.teacher_can_invite&&<Link className="dashboard-tool" href="/teacher-access"><b>Teacher access</b><span>Invite approved teachers</span></Link>}</div>
      </section>

      <div className="row between dashboard-library-heading" style={{alignItems:'end'}}><div><h2>Tests</h2><p className="muted">Reusable test content</p></div><Link href="/tests/new">Build new →</Link></div>
      {!tests?.length?<div className="card empty-state"><h3>No tests yet</h3><p className="muted">Build a reusable test from your Question Bank, then create an assignment when you are ready.</p><Link className="button" href="/tests/new">Build your first test</Link></div>:<div style={{display:'grid',gap:12}}>{tests.map((t:any)=>{const published=t.status==='published';const attempts=Number(t.attempts?.[0]?.count??0);return <section className="card" key={t.id} style={{margin:0,padding:'18px 20px'}}>
        <div className="row between" style={{alignItems:'flex-start',gap:12,flexWrap:'nowrap'}}>
          <div style={{minWidth:0}}><h3 style={{margin:'0 0 2px',fontSize:23,lineHeight:1.2}}>{t.title}</h3><small style={{display:'block',fontWeight:750,color:published?'#3730a3':'#64748b',marginBottom:6}}>{published?'Published':'Archived'}</small><small className="muted">Edited {new Date(t.updated_at).toLocaleDateString()} · {attempts} attempt{attempts===1?'':'s'}</small></div>
          <Link className="button" style={{padding:'8px 11px',fontSize:13,whiteSpace:'nowrap'}} href={`/assignments/new?test=${t.id}`}>Create assignment</Link>
        </div>
        <div className="row" style={{gap:6,marginTop:14,flexWrap:'wrap'}}><Link className="secondary button" style={{padding:'7px 10px',fontSize:13}} href={`/tests/${t.id}/preview`}>Preview</Link><Link className="secondary button" style={{padding:'7px 10px',fontSize:13}} href={`/tests/${t.id}/print`}>🖨 Print</Link><Link className="secondary button" style={{padding:'7px 10px',fontSize:13}} href={`/tests/${t.id}/edit`} aria-label={`Edit ${t.title}`}>✎ Edit</Link><Link className="secondary button" style={{padding:'7px 10px',fontSize:13}} href={`/tests/${t.id}/reports`}>Reports</Link><form action={copyTest.bind(null,t.id)} style={{margin:0}}><button className="secondary" style={{padding:'7px 10px',fontSize:13}} aria-label={`Copy ${t.title}`}>⧉ Copy</button></form></div>
      </section>})}</div>}
    </main>
  }
  ...
  return <main>{query.error&&<p className="bad">{query.error}</p>}
    ...
    <div className="row" style={{flexWrap:'wrap',margin:'14px 0 18px'}}><Link className="button" href="/my-passes">★ My workspace</Link><Link className="secondary button" href="/practice-library">Browse practice bundles</Link></div>
