...
export default async function NewTest({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  return <main><Link href="/dashboard">← Dashboard</Link><div className="row between"><div><h1>Create a test</h1><p className="muted">Choose source banks, chapter buckets, and subject weighting. A multi-tagged question can qualify through several buckets but will only be used once in the test.</p></div><div className="row" style={{flexWrap:'wrap'}}><Link className="secondary button" href="/test-blueprints">Smart blueprints</Link><Link className="secondary button" href="/question-bank">Question bank</Link></div></div>{query.error && <p className="bad">{query.error}</p>}<ClassroomTestBuilder action={createTest} saveMixPresetAction={saveSubjectMixPreset} bankQuestions={bank as any} previousTests={previousTests as any} sourceBuckets={sourceBuckets as any} bundlePresets={bundlePresets as any} initialMixPresets={(mixPresets??[]) as any} /></main>
}
