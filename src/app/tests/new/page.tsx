...
export default async function NewTest({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  const { data: bank } = await supabase.from('question_bank').select('id,prompt,choices,correct_index,content_area,source_type').order('updated_at',{ascending:false}).limit(1000)
