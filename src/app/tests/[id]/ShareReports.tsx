...
export default function ShareReports({testTitle,shares,attempts}:{testTitle:string;shares:ShareRow[];attempts:AttemptRow[]}){
  return <section className={`card ${styles.report}`}>
    <div className={`row between ${styles.reportHeader}`}><div><h2>Assignment reports</h2><p className="muted">Each share is reported separately so the same reusable test can be compared across classes, dates, and retake policies.</p></div><span className="pill">{shares.length} assignment{shares.length===1?'':'s'}</span></div>
    {shares.length===0?<p className="muted">No assignments have been shared yet.</p>:shares.map(share=>{
      ...
      return <details className={styles.card} key={share.id}>
        <summary>
          <div className={styles.heading}><div><span className="eyebrow">{share.label||'Untitled assignment'}</span><h3>{testTitle}</h3></div><span className="pill">{share.active?'Active':'Disabled'}</span></div>
          <div className={styles.stats}>
            ...
            <div className={styles.stat}><span>Student access</span><b>{accessedIds.size}</b></div>
            <div className={styles.stat}><span>Students complete</span><b>{completedIds.size}</b></div>
          </div>
        </summary>
        <div className={styles.body}>
          <div className={`grid two ${styles.meta}`}>
            <section><h3>Share parameters</h3><p><b>Experience:</b> {share.experience_name||mode}</p><p><b>Assigned:</b> {fmt(share.created_at)}</p><p><b>Due:</b> {fmt(share.due_at)}</p><p><b>Audience:</b> {audience}</p><p><b>Test environment:</b> {mode}</p><p><b>Full attempts:</b> {attemptsPolicy}</p><p><b>Focused retake:</b> {focusedPolicy}</p><p><b>Focused hints:</b> {share.focused_retake_hints?'On':'Off'}</p><p><b>Study guide:</b> {share.study_guide_enabled?'On':'Off'}</p><p><b>Fresh full retest:</b> {share.randomized_retest_enabled?'On when another attempt is allowed':'Off'}</p><p><b>Integrity monitoring:</b> {share.restricted_mode?'On':'Off'}</p><p><b>Share-link expiration:</b> {fmt(share.link_expires_at)}</p>{share.payment_mode==='paid'&&<p><b>Paid access:</b> ${((share.price_cents||0)/100).toFixed(2)} · {share.access_duration_days||14} days</p>}</section>
