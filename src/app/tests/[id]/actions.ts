...
export async function createShareOffer(testId:string,fd:FormData){
  ...
  const expiresRaw=String(fd.get('link_expires_at')||'').trim();const guide=fd.get('study_guide_enabled')==='on'||deliveryMode==='study'||paid;const focused=fd.get('focused_retake_enabled')==='on'||requireFocused||deliveryMode==='study'||paid;const randomized=fd.get('randomized_retest_enabled')==='on'||paid
