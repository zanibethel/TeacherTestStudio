...
export default function ShareSetupForm({roster,groups,presets,proActive,fullQuestionCount}:{roster:RosterRow[];groups:GroupRow[];presets:SavedPreset[];proActive:boolean;fullQuestionCount:number}){
  ...
  return <>
    <label>Assignment / share label <span className="muted">(optional)</span></label>
    <input name="label" placeholder="Period 2 final, Week 1 exam, makeup assignment"/>
    ...
    <p className="muted">Choose a preset to fill the rules below. Change any experience rule and this becomes <b>Custom</b>. Assignment-specific fields like due date and audience do not change the experience.</p>
    ...
    <section className="notice"><h3>Deadline & full attempts</h3><div className="settings-grid"><div><label>Full test attempts</label><input name="max_attempts" type="number" min="1" max="100" value={settings.maxAttempts} disabled={settings.unlimited||settings.paidAccess} onChange={e=>change('maxAttempts',Number(e.target.value)||1)}/><p className="muted">2 means the original attempt plus 1 additional full attempt.</p></div><div><label>Due date & time</label><input name="due_at" type="datetime-local"/><p className="muted">Assignment-specific. Optional with limited attempts; required with unlimited attempts.</p></div></div><label className="check"><input type="checkbox" name="unlimited_attempts_until_due" checked={settings.unlimited} disabled={settings.paidAccess} onChange={e=>change('unlimited',e.target.checked)}/><span><b>Unlimited full attempts until due date</b></span></label><p className="muted">When another full attempt is allowed, CramLoop automatically builds a fresh retest from the approved test pool where possible.</p></section>
    ...
    <label>Share-link expiration <span className="muted">(optional)</span></label><input name="link_expires_at" type="datetime-local"/><p className="muted">Assignment-specific. Usually the due date is enough.</p>
