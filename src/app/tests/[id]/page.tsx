...
export default async function TestDetail({params,searchParams}:{params:Promise<{id:string}>,searchParams:Promise<{error?:string;created?:string;token?:string;audience?:string;label?:string;targets?:string;group?:string;step?:string}>}){
 ...
 const assigning=query.step==='assign'||Boolean(query.created)
 return <main><Link href="/dashboard">← Dashboard</Link><div className="row between" style={{alignItems:'flex-start',gap:12,flexWrap:'wrap'}}><div><h1>{test.title}</h1><p className="muted">{test.description||'No summary'}</p></div><div className="row" style={{gap:8,flexWrap:'wrap'}}><Link className="secondary button" href={`/tests/${id}/preview`}>Preview test</Link><span className="pill">{test.status}</span></div></div>{query.error&&<p className="bad">{query.error}</p>}

  <section className="card" style={{padding:'12px 16px'}}><div className="row" style={{gap:8,flexWrap:'wrap',alignItems:'center'}}><span className="pill">1 · Build ✓</span><span aria-hidden>→</span><span className="pill">2 · Review ✓</span><span aria-hidden>→</span><span className="pill">3 · Assign{assigning?'':' / Manage'}</span><span aria-hidden>→</span><span className="muted">4 · Analyze</span></div></section>
  ...
  <section id="assign-test" className="card stack"><div className="row between" style={{alignItems:'flex-start',gap:12,flexWrap:'wrap'}}><div><span className="eyebrow">STEP 3</span><h2 style={{marginTop:4}}>Assign this test</h2><p className="muted">Choose who gets it first, set the classroom details, then pick the testing experience and remediation rules.</p></div><Link className="secondary button" href={`/tests/${id}/preview`}>← Back to review</Link></div>
   {query.created&&createdUrl&&<section id="share-success" className="card" style={{border:'2px solid #86efac',background:'#f0fdf4',margin:0}}><span className="eyebrow">ASSIGNMENT CREATED</span><h3 style={{margin:'6px 0'}}>{query.label||createdShare?.label||'Assignment ready'}</h3><p style={{margin:'4px 0'}}>Assigned to <b>{targetText}</b>.</p><p className="muted" style={{margin:'4px 0 10px'}}>Students in this audience can now access the assignment through their normal CramLoop assignment flow. Create another assignment only if you want different students, dates, or rules.</p><div className="row" style={{flexWrap:'wrap',gap:8}}><Link className="button" href={progressHref}>View student progress</Link><Link className="secondary button" href={`/reports?assignment=${query.created}`}>View assignment report</Link><CopyShareLinkButton url={createdUrl}/><a className="secondary button" href={createdUrl}>Open student link</a></div><p className="muted" style={{wordBreak:'break-all',marginBottom:0}}>{createdUrl}</p></section>}
