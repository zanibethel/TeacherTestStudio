...
export default async function TestDetail({ params, searchParams }: { params: Promise<{ id: string }>, searchParams: Promise<{ error?: string }> }) {
  ...
  return <main>
    ...
    <section className="card"><h2>Student results ({attempts.length})</h2>{attempts.length === 0 ? <p className="muted">No submissions yet.</p> : attempts.map((a:any) => { const student = Array.isArray(a.student) ? a.student[0] : a.student; return <div className="result-row" key={a.id}><div><b>{student?.full_name || 'Student'}</b><p className="muted">{a.submitted_at ? new Date(a.submitted_at).toLocaleString() : 'In progress'}</p></div><div><b>{a.score_percent}%</b><p className="muted">{a.correct_count}/{a.total_questions} correct</p></div><Link href={`/attempts/${a.id}`}>Review</Link></div> })}</section>
  </main>
}
