...
export async function updateStudentRoster(rosterId:string,fd:FormData){
  ...
  const{data,error}=await supabase.rpc('update_teacher_student_roster',{p_roster_id:rosterId,p_email:email,p_first_name:firstName||null,p_last_name:lastName||null,p_group_ids:groupIds,p_class_name:className||null})
  if(error)redirect(`/teacher-roster?error=${encodeURIComponent(error.message)}`)
  revalidatePath('/teacher-roster');revalidatePath('/teacher-groups')
  const groupName=(data as any)?.group_name
...
export async function removeStudent(rosterId:string){const{supabase}=await teacher();const{error}=await supabase.rpc('remove_teacher_student_roster',{p_roster_id:rosterId});if(error)redirect(`/teacher-roster?error=${encodeURIComponent(error.message)}`);revalidatePath('/teacher-roster')}
...
export async function deleteGroup(groupId:string){const{supabase,user}=await teacher();const{error}=await supabase.from('teacher_groups').delete().eq('id',groupId).eq('teacher_id',user.id);if(error)redirect('/teacher-roster?error='+encodeURIComponent(error.message));revalidatePath('/teacher-roster');revalidatePath('/teacher-groups')}
