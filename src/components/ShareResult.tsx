...
export default function ShareResult({score,title,passingScore}:{score:number;title:string;passingScore:number}){
  ...
  const text=passed
    ? `I used Teacher Test Studio to practice and passed ${title} with ${score}%! Think you can beat my score? Try a practice test.`
    : `I used Teacher Test Studio to practice ${title} and scored ${score}%. I’m working on improving my skills — try a practice test too.`
  const url=`${SITE_URL}/practice-library?utm_source=result_share&utm_medium=student_referral`

  async function share(){
    try{
      if(navigator.share){
        await navigator.share({title:'Practice test result',text,url})
        setStatus('Shared!')
        return
      }
  ...
  return <section className="card">
    ...
    <p className="muted">Invite a friend to try practice tests too. Your email, answers, teacher information, and detailed attempt data are never included.</p>
