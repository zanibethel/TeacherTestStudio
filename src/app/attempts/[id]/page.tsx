...
export default async function AttemptReview({params}:{params:Promise<{id:string}>}){
  ...
  const focusAreas=focus.map(([area])=>area).slice(0,3)
  const{data:recommended}=focusAreas.length?await supabase.from('shared_collections').select('id,title,description,subject,collection_type,shared_collection_questions(count)').eq('active',true).eq('student_available',true).in('title',focusAreas.map(a=>`${a} Practice Set`)): {data:[] as any[]}
  return <main>
