...
type AreaStat={total:number;correct:number;missed:number;rows:any[]}
type AreaEntry=[string,AreaStat]
...
export default async function AttemptReview({params,searchParams}:{params:Promise<{id:string}>,searchParams:Promise<{practice_error?:string}>}){
  ...
  const allAreas:AreaEntry[]=(attempt.areas??[]).map((a:any)=>[String(a.area),{total:Number(a.total||0),correct:Number(a.correct||0),missed:Number(a.missed||0),rows:a.rows??[]}])
  const weakAreas:AreaEntry[]=allAreas.filter(([,v])=>v.missed>0);const strongAreas:AreaEntry[]=allAreas.filter(([,v])=>v.missed===0);const focusNames:string[]=weakAreas.map(([area])=>area)
  ...
  return <main className="attempt-results"><Link href={isStudent?'/dashboard':`/tests/${attempt.test_id}`}>← Back</Link><div className="result-heading"><div><p className="eyebrow">ATTEMPT {attempt.attempt_number}</p><h1>{test.title}</h1></div><div className={`result-score ${failed?'needs-work':'passed'}`}><b>{attempt.score_percent}%</b><span>{attempt.correct_count}/{attempt.total_questions}</span></div></div>
    ...
    <section className="card subject-overview"><div className="row between"><div><h2>Subject results</h2><p className="muted">Start here. Open a subject only when you want the question-level detail.</p></div><span className="pill">{allAreas.length} subjects</span></div>
      <div className="subject-list">{allAreas.map(([area,v]:AreaEntry)=>{const pct=v.total?Math.round(v.correct/v.total*100):0;return <details className="subject-result" key={area}><summary><div><b>{area}</b><span>{v.correct} of {v.total} correct</span></div><strong className={pct>=passing?'good':pct<60?'bad':''}>{pct}%</strong></summary>
    ...
    {isStudent&&failed&&focusedEnabled&&<section className="card focused-next" id="focused-retake"><span className="eyebrow">RECOMMENDED NEXT STEP</span><h2>{requiredFocused?'Focused retest required':'Focused retest'}</h2><p className="focused-lead">Your focused retest will <b>mostly consist of these subjects</b>, where you lost points:</p><div className="focus-subject-pills">{weakAreas.map(([area,v]:AreaEntry)=><span className="focus-subject" key={area}><b>{area}</b><small>{v.total?Math.round(v.correct/v.total*100):0}%</small></span>)}</div>
      <form action={createFocusPractice.bind(null,id)} className="focus-form">{query.practice_error&&<p className="bad">{query.practice_error}</p>}{!requiredFocused&&focusNames.map((area:string)=><input key={area} type="hidden" name="areas" value={area}/>)}<input type="hidden" name="question_count" value={requiredFocused?focusedCount:Math.min(20,Math.max(5,focusedCount))}/>
        {strongAreas.length>0&&<details className="select-more"><summary>+ Select more subjects <span>optional</span></summary><p className="muted">Added subjects provide some extra coverage. Your missed subjects remain the main focus.</p>{strongAreas.map(([area,v]:AreaEntry)=><label className="check" key={area}><input type="checkbox" name="areas" value={area}/><span>{area} <span className="muted">· {v.total?Math.round(v.correct/v.total*100):0}%</span></span></label>)}</details>}
