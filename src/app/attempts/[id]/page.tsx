...
export default async function AttemptReview({params}:{params:Promise<{id:string}>}){
  ...
  if(!attempt)notFound()
  const test=Array.isArray(attempt.tests)?attempt.tests[0]:attempt.tests;const share=Array.isArray(attempt.share)?attempt.share[0]:attempt.share;const student=Array.isArray(attempt.student)?attempt.student[0]:attempt.student
  ...
  const areaMap=new Map<string,{total:number;missed:number}>();for(const r of responses){const area=r.questions?.content_area||'General review';const row=areaMap.get(area)||{total:0,missed:0};row.total++;if(!r.is_correct)row.missed++;areaMap.set(area,row)}
